...
PCA가 차원의 축소를 위해 수행하는 알고리즘이라면, 
LDA는 클래스를 구분하기 위해 사용하는 위한 방법이다. 
...
그러면, 우선 투영 후 클래스간의 평균거리를 정의해야 한다. 
그러기 위해선 클래스들의 평균, 분산값들을 알 필요가 있다. 
...
$$
    S_w = \sum_{i} \sum_{x\in C_i} (x-m_i)(x-m_i)^T
$$
...
$$
J(w) = \frac{(w^T(m_1-m_0))^2}{w^T S_w w}
$$

- 분자 = 클래스 평균 간 거리 (between-class)
...
## 다중 클래스에서의 LDA
클래스가 2개 이상인 경우, 각 클래스 평균이 **전체 평균으로부터 얼마나 떨어져 있는가**를 기반으로 정의된다.  
...
### 다중 클래스들사이의 분산 (Between-class Scatter Matrix)
